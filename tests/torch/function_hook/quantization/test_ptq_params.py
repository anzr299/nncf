...
from tests.torch.function_hook.quantization.helper import get_nncf_network
from tests.torch.function_hook.quantization.helper import get_single_conv_nncf_graph
from tests.torch.function_hook.quantization.helper import get_single_no_weight_matmul_nncf_graph
...
class TestPTQParams(TemplateTestPTQParams):
    ...
    def check_unified_scale_layout(self, layout, unified_scale_group):
        ...
        assert isinstance(command, PT2InsertionCommand)
        assert command.target_points == unified_scale_group
        assert torch.allclose(command.hook_module.scale, torch.tensor(4.0))
    ...
    def test_params(self):
        ...
        return {
            "test_range_estimator_per_tensor": {
                "model": linear_model,
                "nncf_graph": linear_model.get_graph(),
                "stat_points_num": 5,
            },
            "test_range_estimator_per_channel": {
                "model": depthwise_model,
                "nncf_graph": depthwise_model.get_graph(),
                "stat_points_num": 2,
            },
